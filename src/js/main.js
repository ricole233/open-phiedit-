...
// 使用全局配置的路径
const notes_path = "./src/img/notes/";
imgs[0].src = notes_path + "Tap.png";
...
function main() {
	...
	if (playing == 1) {
		...
		ctx.fillRect(0, 0, 1600, renderer.y1);  // 上方区域
		ctx.fillRect(0, renderer.y2, 1600, 900); // 下方区域
		ctx.fillRect(0, 0, renderer.x1, 900);    // 左侧区域
		ctx.fillRect(renderer.x2, 0, 1600, 900); // 右侧区域
		...
		function render_text(x, y1, y2) { // 绘制小节的数字
			...
			for (let i = Math.floor(Math.max(-rdelta / hi, 0)); i < n; i++) {
				let y = (i * hi) - 450; // 相对坐标
				y += rdelta; // 应用滚动偏移
				if (y > 500) continue; // 超出屏幕下方，不绘制
				let ty = y2 - ((y + 450) / 900) * (y2 - y1); // 转换为绝对坐标
				ctx.fillText(i, x, ty);
			}
		}
		// 刷新事件层级列表（当事件层数量改变时）
		if (evs.length != r_last_evs_length) {
			...
			let con = $("eventlayer-con"); // 获取层级容器元素
			con.innerHTML = ""; // 清空容器
			// 重新创建所有层级按钮
		...
		// 半透明预览游戏画面（如果启用了播放器预览）
		if (settingscontrol.settings.player == 1) {
			ctx.globalAlpha = 0.4; // 设置半透明
			renderer.pre(); // 预处理渲染数据
			if (renderer) renderer.main([Math.floor(-rdelta / hi), -rdelta % hi, hi]); // 绘制游戏画面
			ctx.globalAlpha = 1; // 恢复不透明
		}

		// 渲染选择框（当用户正在框选音符时）
		if (mousedata.sel_rect.x1 != null) {
			ctx.fillStyle = color.select_rect; // 设置选择框颜色
			// 计算选择框起始y坐标
		...
		for (let i = 0; i < all_data.judgeLineList.length; i++) {
			...
			op.innerHTML =
				`<div class="option-judgeline" data-i="${i}">
					...
					<br>
					<span class="option-judgeline-pos"></span> <!-- 位置信息 -->
					<span class="option-judgeline-deg"></span> <!-- 角度信息 -->
					<span class="option-judgeline-n"></span>   <!-- 音符数量 -->
				</div>
			`;
			// 添加点击事件，切换当前选中的判定线
	...
	for (let i = 0; i < nodes.length; i++) {
		...
		// 获取并格式化当前位置的X和Y坐标
		nodes[i].innerText = "(" + 
		    renderer.getval(all_data.judgeLineList[id], "moveXEvents", -rdelta / hi).toFixed(0) + "," + 
		    renderer.getval(all_data.judgeLineList[id], "moveYEvents", -rdelta / hi).toFixed(0) + ")";
	}
